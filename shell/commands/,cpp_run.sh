...
if [ -z "$1" ]; then
    echo "Usage: cpp-run.sh <filename.cpp>"
    exit 1
fi
...
# Compile the C++ file with all warnings enabled (except -Werror)
if g++ -std=c++20 -Wall -Wextra -pedantic "$1" -o "$TEMP_EXECUTABLE"; then
    # Run the executable
    echo "----------------------------------------"
    echo "Program output:"
    echo "----------------------------------------"
